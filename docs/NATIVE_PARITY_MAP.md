...
| Stream Deck / Companion control surface | `app/setup/deckConfig.ts`, `app/setup/StreamDeckReplica.tsx`                                 | `app/api/deck/*`, `app/api/companion-config`                                                           | engine `control_surface`, `exports`; shell `ui/commissioning/control-surface/*`                   | Late migration  |
...
- Stream Deck and Companion export flows should wait until commissioning, selection state, and core domain ownership are stable in the engine.
